...
export class Bot extends Client {
  ...
  crawler = (p, lvl = 0, ext = ".js") =>
    new fdir()
      ...
      .sync();
  handleEvents = async () => {
    const eventFiles = this.crawler("./events", 1, ".evt.js");
    for (const file of eventFiles) {
      const event = await import(`./${file}`);
      if (event.data.once) {
        this.once(event.data.name, (...args) => event.execute(...args, this));
      } else {
        this.on(event.data.name, (...args) => event.execute(...args, this));
      }
    }
    return this;
  };
  handleButtons = async () => {
    ...
    for (const btnFi of btnFo) {
      const button = await import(`./${btnFi}`);
